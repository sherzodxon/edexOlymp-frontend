...
export default function PptxExamPage() {
  ...
  useEffect(() => {
    ...
    if (user.pptxDone)    { router.replace('/results');   return; }
    if (!user.docsDone)   { router.replace('/exam/docs'); return; }
  ...
  const loadStatus = async (tok: string) => {
    try {
      const res = await api.getPptxStatus(tok) as {
        data: {
          status: string;
      ...
      const total     = data.timeLimitSec ?? 1200;
      ...
      console.error('Load pptx status error:', err);
    }
  };
  ...
  const handleUpload = async () => {
    ...
    try {
      const res = await api.uploadPptx(token, file) as {
        data: { score: number; feedback: FeedbackItem[]; fileName: string };
      };
      cancelAnimationFrame(rafRef.current);

      const user = getStoredUser();
      if (user) saveStoredUser({ ...user, pptxDone: true, pptxScore: res.data.score });
  ...
  const validateFile = (f: File): string | null => {
    if (!f.name.toLowerCase().endsWith('.pptx')) return 'Faqat .pptx formatdagi fayllar qabul qilinadi';
    if (f.size === 0)              return "Fayl bo'sh (0 KB). To'g'ri fayl yuklang";
    if (f.size > 25 * 1024 * 1024) return 'Fayl hajmi 25 MB dan oshmasligi kerak';
    return null;
  };
      ...
      <div className="flex-1 flex items-center justify-center px-4 py-8">
        ...
        {phase === 'expired' && (
          <div className="animate-scale-in text-center max-w-sm space-y-4">
            ...
            <p className="text-sub text-sm">Afsuski, PowerPoint fayl yuklash vaqti tugadi.</p>
            <button
        ...
        {(phase === 'upload' || phase === 'uploading') && (
          <div className="w-full max-w-lg space-y-6 animate-fade-in">
            <div className="text-center">
              <h2 className="text-xl font-semibold text-text mb-1">PowerPoint fayl yuklash</h2>
              <p className="text-sub text-sm">Topshiriqni bajarib, <span className="text-accent font-mono">.pptx</span> faylini yuboring</p>
            </div>
            ...
            >
              ...
              {file ? (
                ...
                <div className="space-y-3">
                  ...
                  <p className="text-muted text-xs">.pptx · maks 25MB</p>
                </div>
              )}
            </div>
        ...
        {phase === 'done' && (
          <div className="animate-scale-in w-full max-w-md space-y-5">
            <div className="bg-surface border border-accent/30 rounded-2xl p-8 text-center space-y-4">
              ...
              <div>
                <p className="text-sub text-sm font-mono mb-1">PowerPoint fayl bali</p>
